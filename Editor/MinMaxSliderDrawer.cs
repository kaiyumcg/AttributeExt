...
namespace AttributeExt
{
    ...
    internal class MinMaxSliderDrawer : PropertyDrawer
    {
        ...
        private float FlexibleFloatFieldWidth(float min, float max, bool hasDecimals, float numberWidth)
        {
            var n = Mathf.Max(Mathf.Abs(min), Mathf.Abs(max));
            float floatFieldWidth = 14f + (Mathf.Floor(Mathf.Log10(Mathf.Abs(n)) + 1) * 8f);
            if (hasDecimals)
            {
                floatFieldWidth += numberWidth;
            }
        ...
        property.propertyType == SerializedPropertyType.Vector2, attr.AmountWidth) :
                kFloatFieldWidth);
